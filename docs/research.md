...
# 3. 建立策略。内置 ma_trend / breakout / weekly_cup_handle / weekly_double_bottom / weekly_platform_hold，默认初始化 ma20 和三项周线策略。
...
- `ma_trend` / `breakout` 仍为日线研究规则。新增 `weekly_cup_handle`：在 8–24 周窗口寻找 12%–45% 勺底，突破周收盘越过左沿且成交量至少为此前四周均量 1.3 倍；随后 1–3 周缩量整理，整理均量不高于突破周 80%、低点不低于突破收盘价 88%；信号周阳线反包整理高点，周量至少为整理均量 1.15 倍。扫描多个窗口后只输出每只股票证据最强的一种组合，并记录实际勺底/整理周数。仅周五收盘形成信号，非周五结束的节假日短周暂不触发；需要至少 160 根日线预热。`min_amount` 对此策略要求反包周成交额至少为配置日门槛的 3 倍。该策略属于研究信号，不继承 MTF-A 的交易许可及执行计划，回测仍受历史 ST/流通市值缺失限制。
- `weekly_double_bottom` 在最近约 26 周内寻找两处相隔 3–10 周的局部低点：第二底相对第一底允许约 -6% 至 +8%，中间颈线至少高于两底 12%，左侧须有跌入第一底的价格空间。第二底成交量不得显著放大，之后不能再破底；只在本周首次放量（相对前四周均量 ≥1.3 倍）阳线收盘突破颈线且不超过颈线 8% 时发信号。信号返回两底日期、价格、颈线和突破量比。沿用周五完整周与 160 根日线预热约束；为研究信号，不承诺次日成交，也不继承 MTF-A 交易许可。
- `weekly_platform_hold` 在 6–20 周窗口内寻找宽度不超过 25%、至少两次分离回踩底部的周线平台，随后要求阳线收盘越过平台上沿至少 1%、突破周量不低于平台末四周均量 1.5 倍。突破后 1–3 周缩量整理：每周收盘不低于平台上沿，周内低点不低于上沿 98%，整理均量不高于突破周 75%，最近一周成交额达到门槛。信号是“突破后守住平台”的观察状态，不要求在整理末周再次放量，也不是立即买入指令；结果含平台宽度/上沿、突破周与量比、整理周数/均量/低点。沿用周五完整周和 160 根日线预热约束。
